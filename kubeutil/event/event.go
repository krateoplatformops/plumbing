...
type Event struct {
	...
	Action  Action         // What action was taken/failed regarding to the regarding object.
}
...
func Normal(r Reason, action Action, message string, opts ...EventOption) Event {
	e := Event{
		Type:    TypeNormal,
		Reason:  r,
		Message: message,
...
func Warning(r Reason, action Action, err error, opts ...EventOption) Event {
	e := Event{
		Type:    TypeWarning,
		Reason:  r,
		Message: err.Error(),
...
func (r *APIRecorder) Event(obj runtime.Object, e Event) {
	r.kube.Eventf(obj, e.Related, string(e.Type), string(e.Reason), string(e.Action), e.Message)
}
